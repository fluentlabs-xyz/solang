...
use crate::{ build_solidity_for_fluentbase};
...
fn callvalue() {
    let mut runtime = build_solidity_for_fluentbase(
        r##"
        contract bar {
            function test_callvalue() public payable {
                 uint128 b = msg.value;
...
fn my_token() {
    #[derive(Debug, PartialEq, Eq, Encode, Decode)]
    struct TokenTest([u8; 20], bool);

    let mut runtime = build_solidity_for_fluentbase(
        "
        contract mytoken {
            function test(address account, bool sender) public payable returns (address) {
                if (sender) {
                    return msg.sender;
                }
                return account;
    ...
    let args = TokenTest(addr, true).encode();
    runtime.function("test", args);
    ...
    runtime.function(
        "test",
        TokenTest(<[u8; 20]>::try_from(&runtime.caller()[..]).unwrap(), true).encode(),
    );
    assert_eq!(&runtime.caller()[..], &runtime.output()[..]);

    runtime.function(
        "test",
        TokenTest(<[u8; 20]>::try_from(&runtime.caller()[..]).unwrap(), false).encode(),
    );
    assert_eq!(&runtime.caller()[..], &runtime.output()[..]);
}

//TODO: Buildin hashes not supported
// #[test]
// fn hash() {
//     let mut runtime = build_solidity_for_fluentbase(
//         r##"
