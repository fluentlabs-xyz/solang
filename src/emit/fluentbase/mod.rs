...
#[macro_export]
macro_rules! emit_fluentbase_context {
    ($binary:expr) => {
        #[allow(unused_macros)]
        ...
        macro_rules! seal_set_storage {
            ($key_ptr:expr, $value_ptr:expr) => {{
                call!("_evm_sstore", &[$key_ptr, $value_ptr])
                    .try_as_basic_value()
                    .right()
                    .map(|_| i32_const!(0))
                    .unwrap()
...
pub struct FluentbaseTarget;

impl FluentbaseTarget {
    pub fn build<'a>(
        context: &'a Context,
        std_lib: &Module<'a>,
        contract: &'a Contract,
        ...
        let mut target = FluentbaseTarget;
    ...
    fn declare_externals(&self, binary: &Binary) {
        ...
        let u64_ptr = ctx.i64_type().ptr_type(AddressSpace::default()).into();
        ...
        external!("_evm_codesize", void_type, u8_ptr);
        external!("_evm_codecopy", void_type, u8_ptr, u8_ptr, u32_val);
        external!("_evm_gasprice", void_type, u8_ptr);
        external!("_evm_number", void_type, u64_ptr);
        ...
        external!("_evm_sstore", void_type, u8_ptr, u8_ptr);
